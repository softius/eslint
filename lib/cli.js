...
const { SuppressedViolationsManager } = require("./services/suppressed-violations-manager");
...
const cli = {
    ...
    async execute(args, text, allowFlatConfig = true) {
        ...
        if (options.suppressAll && options.suppressRule) {
            log.error("The --suppress-all option and the --suppress-rule option cannot be used together.");
            return 2;
        }

        if (options.suppressAll && options.pruneSuppressions) {
            log.error("The --suppress-all option and the --prune-suppressions option cannot be used together.");
            return 2;
        }

        if (options.suppressRule && options.pruneSuppressions) {
            log.error("The --suppress-rule option and the --prune-suppressions option cannot be used together.");
            return 2;
        }
        ...
        if (options.suppressAll || options.suppressRule || options.pruneSuppressions || fs.existsSync(suppressionsFileLocation)) {
            const suppressionsManager = new SuppressedViolationsManager(suppressionsFileLocation, process.cwd());

            if (options.suppressAll) {
                suppressionsManager.suppressAll(results);
            } else if (options.suppressRule) {
                suppressionsManager.suppressByRules(results, options.suppressRule);
            }

            if (options.pruneSuppressions || options.fix) {
                suppressionsManager.prune(results);
            }

            const suppressionResults = suppressionsManager.applySuppressions(results, suppressionsManager.load());

            if (!options.pruneSuppressions && Object.keys(suppressionResults.unused).length > 0) {
                log.error("There are suppressions left that do not occur anymore. Consider re-running the command with `--prune-suppressions`.");
                debug(JSON.stringify(suppressionResults.unused, null, 2));
