...
const cli = {
    ...
    async execute(args, text, allowFlatConfig = true) {
        ...
        if (options.suppressAll || options.suppressRule || options.pruneSuppressions || fs.existsSync(suppressionsFileLocation)) {
            const suppressionsManager = new SuppressionsService({
                filePath: suppressionsFileLocation,
                cwd: process.cwd()
            });

            if (options.suppressAll) {
                await suppressionsManager.suppressAll(results);
            } else if (options.suppressRule) {
                await suppressionsManager.suppressByRules(results, options.suppressRule);
            }

            if (options.pruneSuppressions) {
                await suppressionsManager.prune(results);
            }

            const suppressionResults = suppressionsManager.applySuppressions(results, await suppressionsManager.load());
